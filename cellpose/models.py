...
class CellposeModel(UnetModel):
    ...
    def __init__(self, gpu=False, pretrained_model=False,
        ...
        if model_type is not None or (pretrained_model and not os.path.exists(pretrained_model[0])):
            ...
            pretrained_model = [model_path(
                pretrained_model_string, j, self.torch) for j in model_range]
        ...
        if isinstance(x, list) or x.squeeze().ndim == 5:
            ...
            for i in iterator:
                maski, flowi, stylei = self.eval(x[i],
                                                 ...
                                                 normalize=normalize,
                                                 invert=invert,
                                                 rescale=rescale[i] if isinstance(rescale, list) or isinstance(
                                                     rescale, np.ndarray) else rescale,
            ...
            if not model_loaded and (isinstance(self.pretrained_model, list) and not net_avg and not loop_run):
                self.net.load_model(
                    self.pretrained_model[0], device=self.device)
        ...
        if compute_masks:
            ...
            if do_3D:
                ...
                if stitch_threshold > 0 and nimg > 1:
                    ...
                    masks = utils.stitch3D(
                        masks, stitch_threshold=stitch_threshold)
            ...
            masks, dP, cellprob, p = masks.squeeze(
            ), dP.squeeze(), cellprob.squeeze(), p.squeeze()
    ...
    def train(self, train_data, train_labels, train_files=None,
        ...
        train_flows = dynamics.labels_to_flows(
            train_labels, files=train_files, use_gpu=self.gpu, device=self.device)
        if run_test:
            test_flows = dynamics.labels_to_flows(
                test_labels, files=test_files, use_gpu=self.gpu, device=self.device)
        else:
            test_flows = None
        ...
        model_path = self._train_net(train_data, train_flows,
                                     ...
                                     rescale=rescale, model_name=model_name, tensorboard=tensorboard)
...
class SizeModel():
    ...
    def eval(self, x, channels=None, channel_axis=None,
        ...
        diam_style = self.diam_mean if (
            diam_style == 0 or np.isnan(diam_style)) else diam_style
    ...
    def train(self, train_data, train_labels,
        ...
        diam_train = np.array([utils.diameters(lbl)[0]
                              for lbl in train_labels])
        if run_test:
            diam_test = np.array([utils.diameters(lbl)[0]
        ...
        for iepoch in range(n_epochs):
            ...
            for ibatch in range(0, nimg, batch_size):
                ...
                imgi, lbl, scale = transforms.random_rotate_and_resize([train_data[i] for i in inds],
                                                                       Y=[train_labels[i].astype(
                                                                          np.int16) for i in inds],
                                                                       scale_range=1, xy=(512, 512))

                feat = self.cp.network(imgi)[1]
                styles[inds+nimg*iepoch] = feat
                diams[inds+nimg*iepoch] = np.log(diam_train[inds]) - \
                    np.log(self.diam_mean) + np.log(scale)
            del feat
            if (iepoch+1) % 2 == 0:
        ...
        A = np.linalg.solve(X@X.T + l2_regularization *
                            np.eye(X.shape[0]), X @ y)
        ypred = A @ X
        ...
        if run_test:
            ...
            for i in range(nimg_test):
                styles_test[i] = self.cp._run_net(
                    test_data[i].transpose((1, 2, 0)))[1]
        ...
        self.params = {'A': A, 'smean': smean,
                       'diam_mean': self.diam_mean, 'ymean': ymean}
