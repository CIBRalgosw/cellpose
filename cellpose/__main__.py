...
def main():
    ...
    if len(args.dir) == 0 and len(args.image_path) == 0:
        ...
        device, gpu = models.assign_device(
            use_torch=True, gpu=args.use_gpu, device=args.gpu_device)
        ...
        if not args.train and not args.train_size:
            ...
            if len(args.dir) > 0:
                ...
                if os.path.exists(args.image_path):
                    ...
                    raise ValueError(
                        f'ERROR: no file found at {args.image_path}')
            ...
            if args.diameter == 0:
                if builtin_size:
                    ...
                    logger.info(
                        '>>>> using diameter %0.3f for all images' % diameter)
            else:
            ...
            for image_name in tqdm(image_names, file=tqdm_out):
                ...
                out = model.eval(image, channels=channels, diameter=diameter,
                                 ...
                                 resample=(
                                     not args.no_resample and not args.fast_mode),
                ...
                if not args.no_npy:
                    io.masks_flows_to_seg(
                        image, masks, flows, diams, image_name, channels)
                if saving_something:
            ...
            output = io.load_train_test_data(
                args.dir, test_dir, imf, args.mask_filter, args.unet, args.look_one_level_down)
            ...
            if args.train:
                logger.info(
                    '>>>> during training rescaling images to fixed diameter of %0.1f pixels' % args.diam_mean)
            ...
            if args.train_size:
                ...
                test_masks = [
                    lbl[0] for lbl in test_labels] if test_labels is not None else test_labels
                ...
                if test_images is not None:
                    ...
                    cc = np.corrcoef(predicted_diams, np.array(
                        [utils.diameters(lbl)[0] for lbl in test_masks]))[0, 1]
                    logger.info(
                        'style test correlation: %0.4f; final test correlation: %0.4f' % (ccs, cc))
